...
#[cfg(all(target_os = "macos", not(mynah_no_swift)))]
use log::error;
...
#[cfg(any(not(target_os = "macos"), mynah_no_swift))]
pub fn check_fm_availability() -> String {
...
pub fn fm_clean_ramble(input: String) -> Result<String, String> {
    ...
    #[cfg(all(target_os = "macos", not(mynah_no_swift)))]
    {
        ...
        unsafe {
            ...
            if ptr.is_null() {
                ...
                error!(
                    "[FM] clean_ramble_error error=\"{}\" elapsed_ms={}",
                    err,
            ...
            mynah_fm_free(ptr);
            info!(
                "[FM] clean_ramble_complete input_chars={} output_chars={} elapsed_ms={}",
                input.len(),
                result.len(),
                start.elapsed().as_millis()
            );
    ...
    #[cfg(any(not(target_os = "macos"), mynah_no_swift))]
    {
        ...
        warn!(
            "[FM] clean_ramble_unsupported elapsed_ms={}",
            start.elapsed().as_millis()
        );
        Err(err)
    }
...
pub fn fm_list(input: String) -> Result<String, String> {
    ...
    #[cfg(all(target_os = "macos", not(mynah_no_swift)))]
    {
        ...
        unsafe {
            ...
            if ptr.is_null() {
                ...
                error!(
                    "[FM] list_error error=\"{}\" elapsed_ms={}",
                    err,
                    start.elapsed().as_millis()
                );
                return Err(err);
            ...
            mynah_fm_free(ptr);
            info!(
                "[FM] list_complete input_chars={} output_chars={} elapsed_ms={}",
                input.len(),
                result.len(),
                start.elapsed().as_millis()
            );
    ...
    #[cfg(any(not(target_os = "macos"), mynah_no_swift))]
    {
        ...
        warn!(
            "[FM] list_unsupported elapsed_ms={}",
            start.elapsed().as_millis()
        );
        Err(err)
    }
...
pub fn fm_prompt(input: String) -> Result<String, String> {
    ...
    #[cfg(all(target_os = "macos", not(mynah_no_swift)))]
    {
        ...
        unsafe {
            ...
            if ptr.is_null() {
                ...
                error!(
                    "[FM] prompt_error error=\"{}\" elapsed_ms={}",
                    err,
                    start.elapsed().as_millis()
                );
                return Err(err);
            ...
            mynah_fm_free(ptr);
            info!(
                "[FM] prompt_complete input_chars={} output_chars={} elapsed_ms={}",
                input.len(),
                result.len(),
                start.elapsed().as_millis()
            );
    ...
    #[cfg(any(not(target_os = "macos"), mynah_no_swift))]
    {
        ...
        warn!(
            "[FM] prompt_unsupported elapsed_ms={}",
            start.elapsed().as_millis()
        );
        Err(err)
    }
