...
Codex is removing advanced capture-engine choices from the normal Mynah product surface.
...
### 1. Build Result
🟢 **Passed**
- The web assets compiled successfully with zero syntax, TypeScript, or Svelte compilation issues using `bun run build` inside `apps/mynah` in **6.94s**.
- The native Tauri compilation succeeded in **20.20s** and cleanly packaged the arm64 native `Mynah.app` bundle in `apps/mynah/src-tauri/target/release/bundle/macos/`.

### 2. Test Result
🟢 **Passed**
- The full settings persistence and constraint unit test suite (`bun test apps/mynah/src/lib/state/settings.test.ts`) executed successfully from the workspace root:
...
🟢 **Verified**
- Successfully launched `/Applications/Mynah.app` and checked execution metrics. The app boots in a low-power dormant state in the macOS status tray without listening automatically.
...
- **`AppLayout.svelte`** ([AppLayout.svelte:L57-68](file:///Users/irfan/projects/Mynah/mynah/apps/mynah/src/routes/(app)/_components/AppLayout.svelte#L57-L68)): The migration checks correctly cleanse any stale VAD configs or browser capture modes in Tauri builds.
- **`recording/+page.svelte`** ([recording/+page.svelte:L20-25](file:///Users/irfan/projects/Mynah/mynah/apps/mynah/src/routes/(app)/(config)/settings/recording/+page.svelte#L20-L25)): Correctly filters out the `vad` option under Tauri runtime contexts to prevent user selection.
- **`ManualDeviceSelector.svelte`** ([ManualDeviceSelector.svelte:L75-78](file:///Users/irfan/projects/Mynah/mynah/apps/mynah/src/lib/components/settings/selectors/ManualDeviceSelector.svelte#L75-L78)): Ensures the combobox microphone selection writes straight to CPAL storage keys.
